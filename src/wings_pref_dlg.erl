...
update_normal_dlist(St) ->
    wings_dl:map_dlo(fun(D, _) -> D#dlo{normals=none} end, []),
    wings_draw:refresh_dlists(St).

clear_vertex_dlist() ->
    wings_dl:map_dlo(fun clear_vertex_dlist/2, []).

clear_vertex_dlist(D, _) -> D#dlo{vs=none}.

clear_proxy(St) ->
    wings_dl:map_dlo(fun(D, _) -> clear_proxy(D, St) end, []).
...
clear_proxy_edges(St) ->
    wings_dl:map_dlo(fun(D, _) -> clear_proxy_edges(D, St) end, []).
