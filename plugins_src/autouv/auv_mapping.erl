...
map_chart_1(Type, Chart, Loop, Options, We) ->
	...
	_:Reason ->
	    ...
	    io:format("~p:~p "++?__(3,"Error")++" ~p~n  ~P ~n",
		      [?MODULE,?LINE,Reason,erlang:get_stacktrace(), 40]),
...
map_chart_2(lsqcm, C, Loop, Pinned, We) ->
    R = case get(auv_use_erlang_impl) of
            true ->
                ?TC(lsqcm(C, Pinned, Loop, We)); %% old
            _ ->
                ?TC(lscm(C, Pinned, Loop, We))
        end,
    R;
map_chart_2(harmonic, C, Loop, _Pinned, We) ->
    ?TC(harmonic(C, Loop, We));
